...
### FLEXIBLE DEPTH INTERVAL PATCH START
min_depth <- 0
max_depth <- -7
### FLEXIBLE DEPTH INTERVAL PATCH END
...
# Apply depth interval filter to all datasets
all_datasets <- lapply(all_datasets, function(df) {
  df[df$Bathy_MSL <= min_depth & df$Bathy_MSL >= max_depth, ]
})
...
# Merge in backscatter and coordinates
all_datasets <- lapply(all_datasets, function(df) {
  merge(df, Bathy_10m_backscatter[, c("FID", "Mean.Total", "East", "North")], by.x = "FID_", by.y = "FID", all.x = TRUE)
})

# Spatial train/validation split function
spatial_train_val_split <- function(df, prop_train=0.6, buffer_m=15) {
  df <- df[!is.na(df$East) & !is.na(df$North), ]
  df_sf <- st_as_sf(df, coords = c("East", "North"), crs = 32629, remove=FALSE)
  n_train <- round(nrow(df_sf) * prop_train)
  set.seed(1)
  train_idx <- sample(seq_len(nrow(df_sf)), n_train)
  train_pts <- df_sf[train_idx, ]
  val_pts <- df_sf[-train_idx, ]
  train_buffer <- st_buffer(train_pts, dist = buffer_m)
  buffer_union <- st_union(train_buffer)
  outside_idx <- which(!as.logical(st_intersects(val_pts, buffer_union, sparse=FALSE)))
  train <- as.data.frame(train_pts)
  validation <- as.data.frame(val_pts[outside_idx, ])
  train$geometry <- NULL
  validation$geometry <- NULL
...
for (i in 1:length(dates)) {
  split <- spatial_train_val_split(all_datasets[[i]], prop_train=0.6, buffer_m=15)
  training_datasets[[i]]  <- split$train
  validation_datasets[[i]] <- split$validation
...
training_datasets_Stumpf <- list()
validation_datasets_Stumpf <- list()

for (i in 1:length(dates)) {
  split <- spatial_train_val_split(all_datasets[[i]], prop_train=0.6, buffer_m=15)
  training_datasets_Stumpf[[i]]  <- split$train
  validation_datasets_Stumpf[[i]] <- split$validation
  training_datasets_Stumpf[[i]]$BG <- training_datasets_Stumpf[[i]]$blue/training_datasets_Stumpf[[i]]$green
  validation_datasets_Stumpf[[i]]$BG <- validation_datasets_Stumpf[[i]]$blue/validation_datasets_Stumpf[[i]]$green
  stumpf_2021 <- lm(Bathy_MSL~BG, data = training_datasets_Stumpf[[i]])
  validation_2021 <- validation_datasets_Stumpf[[i]][, c('Bathy_MSL','BG')]
  ...
  RMSE[i] = RMSE(validation_2021$Bathy_MSL, validation_2021$estimated)
  MAE[i] = MAE(validation_2021$Bathy_MSL, validation_2021$estimated)
  MBE[i] = mean(validation_2021$Bathy_MSL - validation_2021$estimated)
...
MAE <- rep(NA, length(all_datasets))
MBE <- rep(NA, length(all_datasets))
AIC_GLM <- rep(NA, length(all_datasets))
...
for (i in 1:length(dates)) {
  split <- spatial_train_val_split(all_datasets[[i]], prop_train=0.6, buffer_m=15)
  training_datasets_GLM[[i]]  <- split$train
  validation_datasets_GLM[[i]] <- split$validation
  glm_2021 <- lm(Bathy_MSL~(blue+green+red)^2, data = training_datasets_GLM[[i]])
  validation_datasets_GLM[[i]]$estimated <- predict(glm_2021, newdata = subset(validation_datasets_GLM[[i]], select= -c(Bathy_MSL)), interval = 'confidence')[,1]
  ...
  AIC_GLM[i] = AIC(glm_2021)
}
metrics_10m_GLM <- data.frame(dates, R2, RMSE, MAE, MBE, AIC_GLM, LE95_GLM)

######################################## Multi-image mean/median analysis (spatial split) ########################################

all_mean_cluster <- bind_rows(all_datasets) %>%
  group_by(FID_, Bathy_MSL) %>%
  summarise(blue = mean(blue), green = mean(green), red = mean(red), East=first(East), North=first(North))
all_median_cluster <- bind_rows(all_datasets) %>%
  group_by(FID_, Bathy_MSL) %>%
  summarise(blue = median(blue), green = median(green), red = median(red), East=first(East), North=first(North))

split_mean <- spatial_train_val_split(all_mean_cluster, prop_train=0.6, buffer_m=15)
training_datasets_mean_comp <- split_mean$train
validation_datasets_mean_comp <- split_mean$validation

split_median <- spatial_train_val_split(all_median_cluster, prop_train=0.6, buffer_m=15)
training_datasets_median_comp <- split_median$train
validation_datasets_median_comp <- split_median$validation
...
metrics_comp <- data.frame("all images", R2_mean, RMSE_mean, MAE_mean, MBE_mean, R2_median, RMSE_median, MAE_median, MBE_median)

# (Continue for all your other blocks, always using spatial_train_val_split for splitting.)
...
for (i in 1:length(dates)) {
  ...
  # plot estimated vs actual for GLM (now uses flexible interval)
  current_label <- paste("Image", i)
  GLM_plots[[i]] <- (
    ggplot(validation_datasets_GLM[[i]], aes(x=estimated ,y=Bathy_MSL)) +
      geom_point(size = 1) + geom_abline(slope=1, intercept=0) + xlab('Estimated depth') +
      ylab('Actual depth') +
      scale_x_continuous(limits = c(max_depth, min_depth), breaks = seq(max_depth, min_depth, by=2)) +
      scale_y_continuous(limits = c(max_depth, min_depth), breaks = seq(max_depth, min_depth, by=2)) +
  ...
  # Residuals vs Fitted Plot for GLM (NEW)
  validation_datasets_GLM[[i]]$residuals <- validation_datasets_GLM[[i]]$Bathy_MSL - validation_datasets_GLM[[i]]$estimated
  validation_datasets_GLM[[i]]$fitted <- validation_datasets_GLM[[i]]$estimated
  resid_plt_glm <- ggplot(validation_datasets_GLM[[i]], aes(x=fitted, y=residuals)) +
    geom_point(size=1) +
    geom_hline(yintercept=0, linetype="dashed", color="red") +
    labs(title=paste("GLM Residuals vs Fitted - Image", i), x="Fitted values", y="Residuals")
  print(resid_plt_glm)
  # Optionally, save:
  # ggsave(filename=paste0("glm_residuals_image_", i, ".png"), plot=resid_plt_glm)
...
### FLEXIBLE DEPTH INTERVAL PATCH START
all_good_datasets <- lapply(all_good_datasets, function(df) {
  df[df$Bathy_MSL <= min_depth & df$Bathy_MSL >= max_depth, ]
})
### FLEXIBLE DEPTH INTERVAL PATCH END
