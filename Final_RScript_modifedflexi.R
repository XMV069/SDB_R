...
# Set your desired depth interval here:
min_depth <- 0     # shallowest (usually 0)
max_depth <- -7   # deepest (e.g., -10 or -7 or any negative number)
...
for (i in 1:length(dates)) {
  ...
  # estimated vs actual scatterplot (now uses flexible interval)
  print(
    ggplot(validation_datasets[[i]], aes(x=estimated ,y=Bathy_MSL)) +
      ...
      geom_abline(slope=1, intercept=-0.5, col="red") +
      geom_text(x=-Inf, y=Inf, hjust=-0.27, vjust=2, label=paste("Image", i), size=6) +
      geom_text(x=-Inf, y=Inf, hjust=-0.3, vjust=3.5, label="Lyzenga", size=6)
  )
  
  # Residuals vs Fitted Plot for Lyzenga (NEW)
  validation_datasets[[i]]$residuals <- validation_datasets[[i]]$Bathy_MSL - validation_datasets[[i]]$estimated
  validation_datasets[[i]]$fitted <- validation_datasets[[i]]$estimated
  resid_plt_lyzenga <- ggplot(validation_datasets[[i]], aes(x=fitted, y=residuals)) +
    geom_point(size=1) +
    geom_hline(yintercept=0, linetype="dashed", color="red") +
    labs(title=paste("Lyzenga Residuals vs Fitted - Image", i), x="Fitted values", y="Residuals")
  print(resid_plt_lyzenga)
  # Optionally, save:
  # ggsave(filename=paste0("lyzenga_residuals_image_", i, ".png"), plot=resid_plt_lyzenga)
...
for (i in 1:length(dates)) {
  ...
  # estimated vs actual scatterplot (now uses flexible interval)
  print(
    ggplot(validation_2021, aes(x=estimated ,y=Bathy_MSL)) +
      geom_point(size = 1) + geom_abline(slope=1, intercept=0) + xlab('Estimated depth') +
      ylab('Actual depth') +
      scale_x_continuous(limits = c(max_depth, min_depth), breaks = seq(max_depth, min_depth, by=2)) +
      scale_y_continuous(limits = c(max_depth, min_depth), breaks = seq(max_depth, min_depth, by=2)) +
      geom_abline(slope=1, intercept=0.5, col="red") +
      geom_abline(slope=1, intercept=-0.5, col="red") +
      geom_text(x=-Inf,y=Inf,hjust=-0.24,vjust=2,label=paste("Image", i), size=6) +
      geom_text(x=-Inf,y=Inf,hjust=-0.3,vjust=3.5,label="Stumpf", size=6)
  )
}
